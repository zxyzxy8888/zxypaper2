...
def save_checkpoint(model, optimizer, args, epoch, acc, sen, spe, auc,
    ...
    save_dir = os.path.join(r'D:\zxyself\output', f'{c1}_vs_{c2}', 'resnet')
...
def main(args):
    ...
    for i in [6]:
        ...
        loss_curve_path = os.path.join(r'D:\zxyself\output', f'{c1}_vs_{c2}', 'resnet', f'i={i}_loss_curve.png')
        ...
        # 保存测试集结果到文件
        result_dir = r"D:\zxyself\output\{0}_vs_{1}\resnet".format(c1, c2)
        ...
        # 保存图形到文件
        plt.savefig(r"D:\zxyself\output\{0}_vs_{1}\resnet\i={2}ROC_output.png".format(c1, c2, i))
        # 清除当前图形（可选，但推荐）
        plt.close()
        fpr_path = r"D:\zxyself\output\{0}_vs_{1}\resnet\i={2}_fpr.mat".format(c1, c2, i)
        tpr_path = r"D:\zxyself\output\{0}_vs_{1}\resnet\i={2}_tpr.mat".format(c1, c2, i)
        ...
        plt.xlabel('UMAP-1')
        plt.ylabel('UMAP-2')
        plt.title('UMAP visualization of Resnet18')
        plt.legend()
        # 保存图片
