...
class Pyramid_1_3_5(nn.Module):
    """1-3-5 配置"""
    def __init__(self, in_ch):
        super().__init__()
        self.conv1 = nn.Conv3d(in_ch, in_ch, 1, padding=0, bias=False)
        self.conv3 = nn.Conv3d(in_ch, in_ch, 3, padding=1, bias=False)
        self.conv5 = nn.Conv3d(in_ch, in_ch, 5, padding=2, bias=False)
        self.bn = nn.BatchNorm3d(in_ch)
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv3d(in_ch * 3, in_ch, kernel_size=1, bias=False)
    def forward(self, x):
        out1 = self.conv1(x)
        out3 = self.conv3(x)
        out5 = self.conv5(x)
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400, input_channels=3, dropout_prob=0.5):
        ...
        self.Pyramid_Convolution = Pyramid_1_3_5(128)
