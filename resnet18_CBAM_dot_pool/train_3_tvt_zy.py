...
from resnet_precls_ch_dot_pool import resnet3d18_cbam_ch_gate
...
c1 = 'AD'
c2 = 'NC'
i = "resnet3d18_cbam_ch_gate_dot_pool"
# Focal Loss 实现
...
def save_checkpoint(model, optimizer, args, epoch, acc, sen, spe, auc,
    ...
    save_dir = os.path.join(r'D:\zxyself\output', f'{c1}_vs_{c2}', 'resnet3d18_cbam_ch_gate_dot_pool')
    ...
    print(f'   Epoch: {epoch}')
    print(f'   Acc: {acc:.2f}, Sen: {sen:.4f}, Spe: {spe:.4f}, AUC: {auc:.4f}')
    print(f'{"=" * 70}\n')
...
def main(args):
    ...
    for i in [6]:
        train_loss = []
        paired_files, subject_to_files = get_paired_files_with_subjects("D:\\zxyself\\zxyself_traindata\\AD_NC\\MRI_Normalized",
                                                                        "D:\\zxyself\\zxyself_traindata\\AD_NC\\PET_Normalized",
                                                                        "D:\\zxyself\\zxyself_traindata\\AD_NC\\mri_2_pet_mapping.csv")
        ...
        torch.manual_seed(1)
        # 创建 DataLoader 实例
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=4,prefetch_factor=2,persistent_workers=True)
        val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4,prefetch_factor=2,persistent_workers=True)
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4,prefetch_factor=2,persistent_workers=True)

        model = resnet3d18_cbam_ch_gate()
        optimizer = torch.optim.AdamW(model.parameters(),lr=args.lr, weight_decay=args.weight_decay)
        start_epoch = 1
        criterion_cls = FocalLoss(alpha=0.75, gamma=2)
        if args.cuda:
        ...
        loss_curve_path = os.path.join(r'D:\zxyself\output', f'{c1}_vs_{c2}', 'resnet3d18_cbam_ch_gate_dot_pool', f'i={i}_loss_curve.png')
        ...
        # 保存测试集结果到文件
        result_dir = r"D:\zxyself\output\{0}_vs_{1}\resnet3d18_cbam_ch_gate_dot_pool".format(c1, c2)
        result_file = os.path.join(result_dir, f'i={i}_test_results.txt')
        ...
        # 保存图形到文件
        plt.savefig(r"D:\zxyself\output\{0}_vs_{1}\resnet3d18_cbam_ch_gate_dot_pool\i={2}ROC_output.png".format(c1, c2, i))
        # 清除当前图形（可选，但推荐）
        plt.close()
        fpr_path = r"D:\zxyself\output\{0}_vs_{1}\resnet3d18_cbam_ch_gate_dot_pool\i={2}_fpr.mat".format(c1, c2, i)
        tpr_path = r"D:\zxyself\output\{0}_vs_{1}\resnet3d18_cbam_ch_gate_dot_pool\i={2}_tpr.mat".format(c1, c2, i)
        ...
        plt.legend()
        # 保存图片
        save_path = os.path.join('D:\zxyself\output',
                                 f'{c1}_vs_{c2}',
                                 'resnet3d18_cbam_ch_gate_dot_pool',
