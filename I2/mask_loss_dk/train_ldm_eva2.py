...
from performance_metric import mean_absolute_error, peak_signal_to_noise_ratio, structural_similarity_index,structural_similarity_index_mask
...
def main():
    ...
    parser.add_argument('--mask_dir', type=str, required=True, help='Mask目录')
    # === 关键参数设置 ===
    parser.add_argument('--batch_size', type=int, default=1, help='物理显存允许的最大Batch Size')
    ...
    # 必须传入 mask_dir
    train_dataset = MRIPETDataset(train_files, mask_dir=args.mask_dir) 
    val_dataset = MRIPETDataset(val_files, mask_dir=args.mask_dir)
    test_dataset = MRIPETDataset(test_files, mask_dir=args.mask_dir)
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for step, batch in progress_bar:
            # 1. 数据加载与预处理
            mri_imgs = normalize_to_neg_one_to_one(batch[0].to(device))
            pet_imgs = normalize_to_neg_one_to_one(batch[1].to(device))
            mask_raw = batch[2].to(device)
            dk_condition = mask_raw / mask_raw.max().clamp(min=1.0)
            condition_input = torch.cat([mri_imgs, dk_condition], dim=1)
            brain_mask = (mri_imgs > 0).float()
            # 2. I2SB 前向传播
            t = torch.randint(0, args.timesteps, (mri_imgs.shape[0],), device=device).long()
            xt = diffusion.q_sample(t, x0=pet_imgs, x1=mri_imgs)
            pred_pet = model(xt, t, cond=condition_input)
            
            # 3. 计算 Loss
            pred_pet = pred_pet * brain_mask
            pet_imgs = pet_imgs * brain_mask
            # loss = F.mse_loss(pred_pet, pet_imgs)
            # loss = F.l1_loss(pred_pet, pet_imgs)
            diff = pred_pet - pet_imgs  # 此时包含背景的误差
            # 3. 计算 Loss (分母只算脑子里的像素个数)
            # 加上 1e-8 是为了防止分母为 0 报错
            loss = (diff ** 2).sum() / (brain_mask.sum() + 1e-8)
        ...
        if (epoch + 1) % args.val_interval == 0:
            ...
            with torch.no_grad():
                for i, val_batch in enumerate(tqdm(val_loader, desc="Validating")):
                    mri_imgs = normalize_to_neg_one_to_one(val_batch[0].to(device))
                    pet_imgs = normalize_to_neg_one_to_one(val_batch[1].to(device))
                    mask_val = val_batch[2].to(device)
                    
                    # 准备条件
                    dk_cond_val = mask_val / mask_val.max().clamp(min=1.0)
                    cond_val = torch.cat([mri_imgs, dk_cond_val], dim=1)
                    # 准备 Loss Mask
                    brain_mask_val = (mri_imgs > 0).float()
                    def pred_x0_fn(xt, t):
                        return model_ema.module(xt, t, cond=cond_val)
                    ...
                    # Masked MAE
                    abs_err = torch.abs(gen_0to1 - pet_0to1) * brain_mask_val
                    mae = abs_err.sum() / (brain_mask_val.sum() + 1e-8)
    ...
    total_mae_mask = 0; total_psnr_mask = 0; total_ssim_mask = 0
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_loader), total=len(test_loader), desc="Test Set Eval"):
            mri_imgs = normalize_to_neg_one_to_one(batch[0].to(device))
            pet_imgs = normalize_to_neg_one_to_one(batch[1].to(device))
            mask_raw = batch[2].to(device) # (B, D, H, W)
            
            # 准备条件
            dk_cond = mask_raw / mask_raw.max().clamp(min=1.0)
            cond = torch.cat([mri_imgs, dk_cond], dim=1)
            
            def pred_x0_fn(xt, t):
                return model(xt, t, cond=cond)
            traj_x, traj_x0 = diffusion.ddpm_sampling(
            ...
            brain_mask = (mri_imgs_0to1 > 0).float()
            pet_0to1 = pet_0to1 * brain_mask
            gen_0to1 = gen_0to1 * brain_mask
            ...
            # MAE/PSNR 分母只除以脑像素数
            num_brain_pixels = brain_mask.sum() + 1e-8
            abs_diff = torch.abs(gen_0to1 - pet_0to1)
            squared_diff = (gen_0to1 - pet_0to1) ** 2
            
            mae_brain = abs_diff.sum() / num_brain_pixels
            mse_brain = squared_diff.sum() / num_brain_pixels
            psnr_brain = 10 * torch.log10(1.0 / mse_brain)
            ssim_brain = structural_similarity_index_mask(pet_0to1, gen_0to1, mask=brain_mask)
            total_mae_mask += mae_brain.item(); total_psnr_mask += psnr_brain.item(); total_ssim_mask += ssim_brain.item()
    ...
    logger.info(f"Test Results - MAE (Brain): {total_mae_mask/n_test:.4f}")
    logger.info(f"Test Results - PSNR (Brain): {total_psnr_mask/n_test:.4f}")
    logger.info(f"Test Results - SSIM (Brain): {total_ssim_mask/n_test:.4f}")
    writer.close()
