...
# 注意：3D Attention 极其消耗显存。如果你的显存 < 24G，建议不要在底层使用 Attention
# 或者将 num_heads 设置为 1
class AttentionBlock(nn.Module):
    def __init__(self, channels, num_heads=1, use_checkpoint=False):
        ...
        self.qkv = conv_nd(1, channels, channels * 3, 1) # Attention 用 1x1 卷积实现
    ...
    def _forward(self, x):
        ...
        weight = torch.einsum(
            "bct,bcs->bts", 
            (q * scale).reshape(b * self.num_heads, ch, -1),
            (k * scale).reshape(b * self.num_heads, ch, -1)  
        )
        weight = torch.softmax(weight.float(), dim=-1).type(weight.dtype)
        a = torch.einsum("bts,bcs->bct", weight, v.reshape(b * self.num_heads, ch, -1)) 
        
        h = a.reshape(b, -1, *spatial[1:]) 
        h = self.proj_out(h.reshape(b, c, -1))
...
class Image3DNet(nn.Module):
    def __init__(self, use_fp16=False,use_attention=False):
        ...
        self.diffusion_model = UNetModel3D(
            ...
            in_channels=3,         # 1 (Noisy PET) + 1 (MRI Condition)
