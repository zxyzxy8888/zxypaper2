...
class Runner(object):
    ...
    def train(self, opt, train_loader, val_loader):
        ...
        log.info(f"Start training for {opt.num_epochs} epochs...")
        net.train()
        for epoch in range(opt.num_epochs):
            ...
            for step, batch_data in enumerate(train_loader):
                optimizer.zero_grad()
                mri, pet, class_id, mri_path, pet_path = batch_data
            ...
            # 每个 epoch 记录一次平均训练损失（横轴为 epoch）
            avg_loss = running_loss / max(num_batches, 1)
            log.info(f"Epoch {epoch+1}/{opt.num_epochs} | LR: {optimizer.param_groups[0]['lr']:.2e} | Train Loss: {avg_loss:.5f}")
            self.writer.add_scalar(epoch + 1, 'train/loss', torch.tensor(avg_loss))

            # 每 3 个 epoch 进行一次验证
            if (epoch + 1) % 3 == 0:
                val_stats = self.validate(opt, epoch + 1, val_loader)
                val_mae = val_stats["mae"]

                if opt.global_rank == 0:
                    self.writer.add_scalar(epoch + 1, 'val/MAE', torch.tensor(val_mae))
                    self.writer.add_scalar(epoch + 1, 'val/PSNR', torch.tensor(val_stats["psnr"]))
                    self.writer.add_scalar(epoch + 1, 'val/SSIM', torch.tensor(val_stats["ssim"]))
    ...
    # -------------------------------------------------------------------------
    @torch.no_grad()
    def validate(self, opt, epoch, val_loader):
        self.log.info(f"=== Validating at epoch {epoch} ===")
        self.net.eval()
        ...
        with self.ema.average_parameters():
            for i, batch in enumerate(val_loader):
                ...
                if i == 0 and opt.global_rank == 0:
                    self.log_visuals(epoch, mri, pet, pred_pet, suffix="val")
        ...
        if opt.global_rank == 0:
            self.log.info(
                f"Validation (epoch={epoch}, nfe={eval_nfe}) | MAE: {avg_mae:.4f}, PSNR: {avg_psnr:.4f}, SSIM: {avg_ssim:.4f}"
            )
            
        return {"mae": avg_mae, "psnr": avg_psnr, "ssim": avg_ssim}

    def log_visuals(self, epoch, mri, pet, pred, suffix=""):
            """ 记录 3D 医疗影像的三正交切面到 TensorBoard """
            # 1. 反归一化到 [0, 1]
            mri = denormalize(mri)
            pet = denormalize(pet)
            ...
            # 2. 获取 3D 维度的中心点坐标
            # 此时 shape 为 [B, C, D, H, W]，例如 [1, 1, 96, 96, 96]
            _, _, D, H, W = mri.shape
            mid_d, mid_h, mid_w = D // 2, H // 2, W // 2

            # 3. 提取三个切面 (仅取 Batch 中的第 0 个样本)
            # 横断面 (Axial) - 沿 D 切
            ax_mri  = mri[0:1, :, mid_d, :, :]
            ax_pet  = pet[0:1, :, mid_d, :, :]
            ax_pred = pred[0:1, :, mid_d, :, :]

            # 冠状面 (Coronal) - 沿 H 切
            cor_mri  = mri[0:1, :, :, mid_h, :]
            cor_pet  = pet[0:1, :, :, mid_h, :]
            cor_pred = pred[0:1, :, :, mid_h, :]

            # 矢状面 (Sagittal) - 沿 W 切
            sag_mri  = mri[0:1, :, :, :, mid_w]
            sag_pet  = pet[0:1, :, :, :, mid_w]
            sag_pred = pred[0:1, :, :, :, mid_w]

            # 4. 拼接矩阵
            # 行排布：第一行横断，第二行冠状，第三行矢状
            # 列排布：MRI (输入) | Real PET (真值) | Fake PET (生成)
            display = torch.cat([
                ax_mri, ax_pet, ax_pred,
                cor_mri, cor_pet, cor_pred,
                sag_mri, sag_pet, sag_pred
            ], dim=0)
            
            # 5. 生成网格 (nrow=3 表示每行 3 张图)
            grid = tu.make_grid(display, nrow=3, padding=2, normalize=False)
            self.writer.add_image(epoch, f'visuals_3planes/{suffix}', grid)
