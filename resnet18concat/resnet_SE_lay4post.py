...
class BasicBlock3D(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None, use_se=False):
        ...
        self.se = SELayer3D(planes, reduction=16) if use_se else None
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400, input_channels=3, dropout_prob=0.5):
        ...
        # 在第四层的残差块内部启用 SE
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, apply_se=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, apply_se=False):
        downsample = None
        ...
        layers = [block(self.in_planes, planes, stride, downsample, use_se=apply_se)]
        self.in_planes = planes * block.expansion

        for _ in range(1, blocks):
            layers.append(block(self.in_planes, planes, use_se=apply_se))
...
class dual_ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=2, input_channels=1, dropout_prob=0.3):
        ...
        self.conv_final = nn.Conv3d(1024, 512, kernel_size=3, stride=1, padding=1)
        self.bn_final = nn.BatchNorm3d(512)
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.dropout = nn.Dropout(p=dropout_prob)
