...
from QKV2 import CrossModalAttention3D
from selfattention import SelfAttention3D
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400, input_channels=3, dropout_prob=0.4):
        ...
        self.layer1 = self._make_layer(block, 32, layers[0], stride=1)   # 原为64
        self.layer2 = self._make_layer(block, 64, layers[1], stride=2)   # 原为128
        self.layer3 = self._make_layer(block, 128, layers[2], stride=2)  # 原为256
        self.layer4 = self._make_layer(block, 256, layers[3], stride=2) # 原为512
        self.cbam = CBAM3D(256 * block.expansion)  # 在最后一个残差块后添加CBAM模块
...
class dual_ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=2, input_channels=1, dropout_prob=0.3):
        ...
        self.self_QKV1=SelfAttention3D(256)
        self.self_QKV2=SelfAttention3D(256)
        self.QKV=CrossModalAttention3D(256)
        self.conv_final = nn.Conv3d(1024, 128, kernel_size=1, stride=1, padding=1)
        self.bn_final = nn.BatchNorm3d(128)
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.gate = ChannelGatedFusion(feature_dim=256, hidden_ratio=0.5)
    ...
    def forward(self, x):
        ...
        out1 = self.res1(x1)
        out2 = self.res2(x2)
        # 对每个模态先做自注意力（返回两个张量，第二个与第一个相同，这里只取第一个）
        out1= self.self_QKV1(out1)
        out2= self.self_QKV2(out2)
        # 再进行跨模态注意力融合，得到最终增强特征
        out1, out2 = self.QKV(out1, out2)
        
        # 全局平均池化，将3D特征图转换为1D特征向量
        out1_pooled = self.avgpool(out1)  # [B, 512, 1, 1, 1]
        out2_pooled = self.avgpool(out2)  # [B, 512, 1, 1, 1]
        out1_pooled = torch.flatten(out1_pooled, 1)  # [B, 512]
        out2_pooled = torch.flatten(out2_pooled, 1)  # [B, 512]
        
        # 门控融合
        out = self.gate(out1_pooled, out2_pooled)  # [B, 512]
