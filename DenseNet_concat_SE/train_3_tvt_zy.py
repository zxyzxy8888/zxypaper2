...
from model import DenseNet3D_SE_T3
...
def save_checkpoint(model, optimizer, args, epoch, acc, sen, spe, auc,
    ...
    save_dir = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet3D_SE_T3')
...
def main(args):
    ...
    for i in [6]:
        ...
        model = DenseNet3D_SE_T3(in_channels=2, num_classes=2)
        ...
        loss_curve_path = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet3D_SE_T3', f'i={i}_loss_curve.png')
        ...
        # 保存测试集结果到文件
        result_dir = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet3D_SE_T3".format(c1, c2)
        ...
        # 保存图形到文件
        plt.savefig(r"D:\zxyself\pic\{0}_vs_{1}\DenseNet3D_SE_T3\i={2}ROC_output.png".format(c1, c2, i))
        # 清除当前图形（可选，但推荐）
        plt.close()
        fpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet3D_SE_T3\i={2}_fpr.mat".format(c1, c2, i)
        tpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet3D_SE_T3\i={2}_tpr.mat".format(c1, c2, i)
