...
class MFNET_3D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False, **kwargs):
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ('conv',
             nn.Conv3d(2, conv1_num_out, kernel_size=(3, 5, 5), padding=(1, 2, 2), stride=(1, 2, 2), bias=False)),
    ...
    def forward(self, x):
        ...
        futures = self.pred1(h)
        h = self.pred2(futures)
...
#     c1 = 'AD'
#     c2 = 'MCI'
#     dataset, label, roi_data = load_dataset(d=['ADNI'], c1=c1, c2=c2, h='cropped',
#                                             m=['GM', 'WM', 'PET'])
#
#     c1 = 'AD'
#     c2 = 'MCI'
...
#     # 假设dataset形状为(n, 3, 121, 145, 121)
#     data = dataset[:, :, 57:63, :, :]
#
#     logging.getLogger().setLevel(logging.DEBUG)
#     # ---------
#     net = MFNET_3D(num_classes=2, pretrained=False).to(device='cuda')
#     data = torch.autograd.Variable(torch.randn(1, 2, 16, 224, 224)).to(device='cuda')
#     summary(net, input_size=(2, 16, 224, 224), batch_size=1, device='cuda')
#     output = net(data)
