...
def save_checkpoint(model, optimizer, args, epoch, acc, sen, spe, auc,
    ...
    save_dir = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet_concat')
...
def main(args):
    ...
    for i in [6]:
        ...
        start_epoch = 1
        criterion_cls = FocalLoss(alpha=0.5, gamma=2.0)
        if args.cuda:
        ...
        loss_curve_path = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet_concat', f'i={i}_loss_curve.png')
        ...
        # 保存测试集结果到文件
        result_dir = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat".format(c1, c2)
        ...
        # 保存图形到文件
        plt.savefig(r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat\i={2}ROC_output.png".format(c1, c2, i))
        # 清除当前图形（可选，但推荐）
        plt.close()
        fpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat\i={2}_fpr.mat".format(c1, c2, i)
        tpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat\i={2}_tpr.mat".format(c1, c2, i)
