...
c1 = 'AD'
c2 = 'NC'
i = "DenseNet_concat_CBAM"
# Focal Loss 实现
class FocalLoss(nn.Module):
    """Focal Loss 用于处理类别不平衡问题
    
    适用于模型输出的原始logits（未经过softmax）
    
    Reference: https://arxiv.org/abs/1708.02002
    
    Args:
        alpha (float): 平衡参数，推荐值为0.25
        gamma (float): 焦点参数，推荐值为2.0
    ...
    def forward(self, inputs, targets):
        """
        Args:
            inputs: (N, C) 模型输出的logits，维度等于类别数
            targets: (N,) 目标标签
        """
        # 计算交叉熵损失
        ...
        # 获取预测的概率
        p = torch.exp(-ce_loss)
        # 计算Focal Loss: FL(pt) = -alpha * (1 - pt)^gamma * log(pt)
...
def save_checkpoint(model, optimizer, args, epoch, acc, sen, spe, auc,
    ...
    save_dir = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet_concat_CBAM')
...
def main(args):
    ...
    for i in [6]:
        ...
        model = DenseNet3D(in_channels=2, num_classes=2)
        ...
        loss_curve_path = os.path.join(r'D:\zxyself\pic', f'{c1}_vs_{c2}', 'DenseNet_concat_CBAM', f'i={i}_loss_curve.png')
        ...
        # 保存测试集结果到文件
        result_dir = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat_CBAM".format(c1, c2)
        ...
        # 保存图形到文件
        plt.savefig(r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat_CBAM\i={2}ROC_output.png".format(c1, c2, i))
        # 清除当前图形（可选，但推荐）
        plt.close()
        fpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat_CBAM\i={2}_fpr.mat".format(c1, c2, i)
        tpr_path = r"D:\zxyself\pic\{0}_vs_{1}\DenseNet_concat_CBAM\i={2}_tpr.mat".format(c1, c2, i)
        ...
        plt.legend()
        # 保存图片
        save_path = os.path.join('D:\zxyself\pic',
                                 f'{c1}_vs_{c2}',
                                 'DenseNet_concat_CBAM',
